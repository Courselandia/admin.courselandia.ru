  ...
  <Card :bordered="false">
    ...
    >
      <template #bodyCell="{ column, record }">
        ...
        <template v-if="column.key === 'direction-name'">
          <template v-if="record?.direction">
...
const columns = computed<ITableColumnType<ICollection>[]>(() => [
  {
  ...
  {
    title: lang('collection.direction'),
    ...
    customFilterDropdown: true,
    sortOrder: stateColumnSort('direction-name', sortedInfo.value),
...
const toFilterFields: Record<string, string> = {
  'direction-name': 'directions-id',
};

filteredInfo.value = stateFilters<ICollection>(columns.value, null, null, toFilterFields);
...
const selected = ref<Array<ICollection>>();
...
const rowSelection: TableProps['rowSelection'] = {
  onChange: (selectedRowKeys: Key[], selectedRows: ICollection[]): void => {
...
sortedInfo.value = stateSorts<ICollection>(columns.value, defaultSorts);
...
watch(route, (): void => {
  const currentRoute = router.getRoutes().find((item) => item.name === 'Collections');
  ...
  if (route.path === currentRoute?.path) {
    ...
    sortedInfo.value = stateSorts<ICollection>(columns.value);
    filteredInfo.value = stateFilters<ICollection>(columns.value);
...
const onChange: TableProps<ICollection>['onChange'] = async (pag, filter, sorter): Promise<void> => {
  filteredInfo.value = {
    ...filteredInfo.value,
    ...filter,
  };
