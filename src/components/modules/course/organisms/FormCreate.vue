...
const learnItems: Ref<ILearn[]> = ref([]);
const learnEditableData: UnwrapRef<Record<TId, ILearn>> = reactive({});
...
const employmentItems: Ref<IEmployment[]> = ref([]);
const employmentEditableData: UnwrapRef<Record<TId, IEmployment>> = reactive({});
...
const featureItems: Ref<IFeature[]> = ref([]);
const featureEditableData: UnwrapRef<Record<TId, IFeature>> = reactive({});
...
const onSubmit = async (): Promise<void> => {
  ...
  try {
    ...
    Object.values(learnItems.value).forEach((item) => {
      if (form.value.learns && item.text) {
        form.value.learns[form.value.learns.length] = item.text;
      }
    });
    ...
    Object.values(employmentItems.value).forEach((item) => {
      if (form.value.employments && item.text) {
        form.value.employments[form.value.employments.length] = item.text;
      }
    });
    ...
    Object.values(featureItems.value).forEach((item) => {
      if (form.value.features && item.icon && item.text) {
...
const employmentSave = (id: TId): void => {
...
const featureEdit = (id: TId, field: string): void => {
...
const onClickDeleteFeature = (id: TId): void => {
