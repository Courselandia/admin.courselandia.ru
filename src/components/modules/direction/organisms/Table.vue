...
const columns = computed<ITableColumnType<IDirection>[]>(() => [
  ...
  {
    title: lang('direction.nameField'),
    dataIndex: 'name',
    key: 'name',
    sorter: {
      multiple: 1,
  ...
  {
    title: lang('direction.weight'),
    dataIndex: 'weight',
    key: 'weight',
    sorter: {
      multiple: 1,
    },
    sortOrder: stateColumnSort('weight', sortedInfo.value),
    width: 200,
  },
  {
...
filteredInfo.value = stateFilters<IDirection>(columns.value);
...
const selected = ref<Array<IDirection>>();
const destroySelectedDisabled = ref(true);
const destroySelectedLoading = ref(false);

const rowSelection: TableProps['rowSelection'] = {
  onChange: (selectedRowKeys: Key[], selectedRows: IDirection[]): void => {
...
sortedInfo.value = stateSorts<IDirection>(columns.value, defaultSorts);
...
watch(route, (): void => {
  const currentRoute = router.getRoutes().find((item) => item.name === 'Directions');
  ...
  if (route.path === currentRoute?.path) {
    ...
    sortedInfo.value = stateSorts<IDirection>(columns.value);
    filteredInfo.value = stateFilters<IDirection>(columns.value);
...
const onChange: TableProps<IDirection>['onChange'] = async (pag, filter, sorter): Promise<void> => {
  filteredInfo.value = {
    ...filteredInfo.value,
    ...filter,
  };
