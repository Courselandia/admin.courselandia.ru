...
const columns = computed<ITableColumnType<IEmployment>[]>(() => [
  ...
  {
    title: lang('employment.nameField'),
    dataIndex: 'name',
    key: 'name',
    sorter: {
      multiple: 1,
...
filteredInfo.value = stateFilters<IEmployment>(columns.value);
...
const selected = ref<Array<IEmployment>>();
const destroySelectedDisabled = ref(true);
const destroySelectedLoading = ref(false);

const rowSelection: TableProps['rowSelection'] = {
  onChange: (selectedRowKeys: Key[], selectedRows: IEmployment[]): void => {
...
sortedInfo.value = stateSorts<IEmployment>(columns.value, defaultSorts);
...
watch(route, (): void => {
  const currentRoute = router.getRoutes().find((item) => item.name === 'Employments');
  ...
  if (route.path === currentRoute?.path) {
    ...
    sortedInfo.value = stateSorts<IEmployment>(columns.value);
    filteredInfo.value = stateFilters<IEmployment>(columns.value);
...
const onChange: TableProps<IEmployment>['onChange'] = async (pag, filter, sorter): Promise<void> => {
  filteredInfo.value = {
    ...filteredInfo.value,
    ...filter,
  };
