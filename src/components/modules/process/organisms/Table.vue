...
filteredInfo.value = stateFilters<IProcess>(columns.value);
...
const selected = ref<Array<IProcess>>();
const destroySelectedDisabled = ref(true);
const destroySelectedLoading = ref(false);

const rowSelection: TableProps['rowSelection'] = {
  onChange: (selectedRowKeys: Key[], selectedRows: IProcess[]): void => {
...
sortedInfo.value = stateSorts<IProcess>(columns.value, defaultSorts);
...
watch(route, (): void => {
  const currentRoute = router.getRoutes().find((item) => item.name === 'Processes');
  ...
  if (route.path === currentRoute?.path) {
    ...
    sortedInfo.value = stateSorts<IProcess>(columns.value);
    filteredInfo.value = stateFilters<IProcess>(columns.value);
...
const onChange: TableProps<IProcess>['onChange'] = async (pag, filter, sorter): Promise<void> => {
  filteredInfo.value = {
    ...filteredInfo.value,
    ...filter,
  };
