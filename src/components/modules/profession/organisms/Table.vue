...
const columns = computed<ITableColumnType<IProfession>[]>(() => [
  ...
  {
    title: lang('profession.nameField'),
    dataIndex: 'name',
    key: 'name',
    sorter: {
      multiple: 1,
...
filteredInfo.value = stateFilters<IProfession>(columns.value);
...
const selected = ref<Array<IProfession>>();
const destroySelectedDisabled = ref(true);
const destroySelectedLoading = ref(false);

const rowSelection: TableProps['rowSelection'] = {
  onChange: (selectedRowKeys: Key[], selectedRows: IProfession[]): void => {
...
sortedInfo.value = stateSorts<IProfession>(columns.value, defaultSorts);
...
watch(route, (): void => {
  const currentRoute = router.getRoutes().find((item) => item.name === 'Professions');
  ...
  if (route.path === currentRoute?.path) {
    ...
    sortedInfo.value = stateSorts<IProfession>(columns.value);
    filteredInfo.value = stateFilters<IProfession>(columns.value);
...
const onChange: TableProps<IProfession>['onChange'] = async (pag, filter, sorter): Promise<void> => {
  filteredInfo.value = {
    ...filteredInfo.value,
    ...filter,
  };
