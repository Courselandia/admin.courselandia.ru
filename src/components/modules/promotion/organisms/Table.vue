...
const columns = computed<ITableColumnType<IPromotion>[]>(() => [
  {
  ...
  {
    title: lang('promotion.applicable'),
...
filteredInfo.value = stateFilters<IPromotion>(columns.value, null, null, toFilterFields);
...
const selected = ref<Array<IPromotion>>();
const destroySelectedDisabled = ref(true);
const destroySelectedLoading = ref(false);

const rowSelection: TableProps['rowSelection'] = {
  onChange: (selectedRowKeys: Key[], selectedRows: IPromotion[]): void => {
...
sortedInfo.value = stateSorts<IPromotion>(columns.value, defaultSorts);
...
watch(route, (): void => {
  const currentRoute = router.getRoutes().find((item) => item.name === 'Promotions');
  ...
  if (route.path === currentRoute?.path) {
    ...
    sortedInfo.value = stateSorts<IPromotion>(columns.value);
    filteredInfo.value = stateFilters<IPromotion>(columns.value);
...
const onChange: TableProps<IPromotion>['onChange'] = async (pag, filter, sorter): Promise<void> => {
  filteredInfo.value = {
    ...filteredInfo.value,
    ...filter,
  };
