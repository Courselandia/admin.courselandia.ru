    ...
    >
      <template #bodyCell="{ column, record }">
        ...
        <template v-if="column.key === 'published_at'">
          {{ dayjs.utc(record.published_at).tz(dayjs.tz.guess()).format('D MMMM YYYY, HH:mm:ss') }}
        </template>
      </template>
      <template
...
const columns = computed<ITableColumnType<IPublication>[]>(() => [
  ...
  {
    title: lang('publication.publishedAt'),
    dataIndex: 'published_at',
    key: 'published_at',
    sorter: {
      multiple: 1,
    },
    sortOrder: stateColumnSort('published_at', sortedInfo.value),
    customFilterDropdown: true,
    filterType: 'dateRange',
    filteredValue: stateColumnFilter('published_at', filteredInfo.value, 'dateRange'),
    width: 300,
  },
  {
...
filteredInfo.value = stateFilters<IPublication>(columns.value);
...
const selected = ref<Array<IPublication>>();
...
const rowSelection: TableProps['rowSelection'] = {
  onChange: (selectedRowKeys: Key[], selectedRows: IPublication[]): void => {
...
const defaultSorts: Array<SorterResult> = [
  {
    columnKey: 'published_at',
    order: 'descend',
  },
];
sortedInfo.value = stateSorts<IPublication>(columns.value, defaultSorts);
...
watch(route, (): void => {
  const currentRoute = router.getRoutes().find((item) => item.name === 'Publications');
  ...
  if (route.path === currentRoute?.path) {
    ...
    sortedInfo.value = stateSorts<IPublication>(columns.value);
    filteredInfo.value = stateFilters<IPublication>(columns.value);
...
const onChange: TableProps<IPublication>['onChange'] = async (pag, filter, sorter): Promise<void> => {
  filteredInfo.value = {
    ...filteredInfo.value,
    ...filter,
  };
