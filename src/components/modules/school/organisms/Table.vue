  ...
  <Card :bordered="false">
    ...
    >
      <template #bodyCell="{ column, record }">
        ...
        <template v-if="column.key === 'image_logo_id'">
          <Image
            v-if="record?.image_logo_id?.path"
            :src="record?.image_logo_id?.path"
            :preview="false"
            placeholder
            :width="130"
          />
...
const columns = computed<ITableColumnType<ISchool>[]>(() => [
  ...
  {
    title: lang('school.imageLogo'),
    dataIndex: 'image_logo_id',
    key: 'image_logo_id',
    sorter: false,
    width: 170,
  },
  {
    title: lang('school.name'),
    dataIndex: 'name',
    key: 'name',
    sorter: {
      multiple: 1,
...
filteredInfo.value = stateFilters<ISchool>(columns.value);
...
const selected = ref<Array<ISchool>>();
const destroySelectedDisabled = ref(true);
const destroySelectedLoading = ref(false);

const rowSelection: TableProps['rowSelection'] = {
  onChange: (selectedRowKeys: Key[], selectedRows: ISchool[]): void => {
...
sortedInfo.value = stateSorts<ISchool>(columns.value, defaultSorts);
...
watch(route, (): void => {
  const currentRoute = router.getRoutes().find((item) => item.name === 'Schools');
  ...
  if (route.path === currentRoute?.path) {
    ...
    sortedInfo.value = stateSorts<ISchool>(columns.value);
    filteredInfo.value = stateFilters<ISchool>(columns.value);
...
const onChange: TableProps<ISchool>['onChange'] = async (pag, filter, sorter): Promise<void> => {
  filteredInfo.value = {
    ...filteredInfo.value,
    ...filter,
  };
