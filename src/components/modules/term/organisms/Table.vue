...
const columns = computed<ITableColumnType<ITerm>[]>(() => [
  {
  ...
  },
  {
    ...
    customFilterDropdown: true,
    sortOrder: stateColumnSort('variant', sortedInfo.value),
    filteredValue: stateColumnFilter('variant', filteredInfo.value, 'string'),
  },
  {
    title: lang('term.term'),
    dataIndex: 'term',
    key: 'term',
    sorter: {
      multiple: 1,
    ...
    customFilterDropdown: true,
    sortOrder: stateColumnSort('term', sortedInfo.value),
    filteredValue: stateColumnFilter('term', filteredInfo.value, 'string'),
  },
  {
...
filteredInfo.value = stateFilters<ITerm>(columns.value);
...
sortedInfo.value = stateSorts<ITerm>(columns.value, defaultSorts);
...
watch(route, (): void => {
  const currentRoute = router.getRoutes().find((item) => item.name === 'Terms');
  ...
  if (route.path === currentRoute?.path) {
    ...
    sortedInfo.value = stateSorts<ITerm>(columns.value);
    filteredInfo.value = stateFilters<ITerm>(columns.value);
...
const onChange: TableProps<ITerm>['onChange'] = async (pag, filter, sorter): Promise<void> => {
  filteredInfo.value = {
    ...filteredInfo.value,
    ...filter,
  };
