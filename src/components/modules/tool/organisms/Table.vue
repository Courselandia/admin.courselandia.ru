...
const columns = computed<ITableColumnType<ITool>[]>(() => [
  {
...
filteredInfo.value = stateFilters<ITool>(columns.value);
...
sortedInfo.value = stateSorts<ITool>(columns.value, defaultSorts);
...
watch(route, (): void => {
  const currentRoute = router.getRoutes().find((item) => item.name === 'Tools');
  ...
  if (route.path === currentRoute?.path) {
    ...
    sortedInfo.value = stateSorts<ITool>(columns.value);
    filteredInfo.value = stateFilters<ITool>(columns.value);
...
const onChange: TableProps<ITool>['onChange'] = async (pag, filter, sorter): Promise<void> => {
  filteredInfo.value = {
    ...filteredInfo.value,
    ...filter,
  };
